...
inline std::string GetHardwareArchitecture() {
	int isArm = 0;
...
inline std::string GetEffectiveArchitecture() {
...
inline bool ExecuteCommand(const std::string& command, std::string& output) {
...
inline std::string GetLatestRobloxVersion() {
	...
	if (response.status_code != 200) {
		LOG_ERROR(std::format("Failed to fetch version: HTTP {}", response.status_code));
		return "";
	}
...
inline MacsploitVersion GetMacsploitVersion() {
    ...
    if (response.status_code != 200) {
        LOG_ERROR(std::format("Failed to fetch Macsploit version: HTTP {}", response.status_code));
    ...
    try {
        ...
        LOG_ERROR(std::format("Failed to parse Macsploit version: {}", e.what()));
        throw std::runtime_error(e.what());
    }
}

inline HydrogenVersion GetHydrogenVersion() {
    ...
    if (response.status_code != 200) {
        LOG_ERROR(std::format("Failed to fetch Hydrogen version: HTTP {}", response.status_code));
        throw std::runtime_error(std::format("HTTP {}", response.status_code));
    }
    ...
    try {
        ...
        LOG_ERROR(std::format("Failed to parse Hydrogen version: {}", e.what()));
        throw std::runtime_error(e.what());
    }
}

inline std::string GetDeltaVersion() {
    ...
    if (response.status_code != 200) {
        LOG_ERROR(std::format("Failed to fetch Delta version: HTTP {}", response.status_code));
        throw std::runtime_error(std::format("HTTP {}", response.status_code));
    }
...
inline bool DownloadRoblox(const std::string& version, const std::string& outputPath,
    ...
    LOG_INFO(std::format("Downloading Roblox {} from {}", version, url));
...
inline bool ExtractRoblox(const std::string& zipPath, const std::string& extractTo,
                         ProgressCallback progressCb) {
    if (progressCb) {
    ...
    if (ec) {
        LOG_ERROR(std::format("Failed to create directory: {}", ec.message()));
        return false;
    }
    ...
    if (!ExecuteCommand(command, output)) {
        LOG_ERROR(std::format("Failed to extract: {}", output));
        return false;
    }
...
inline bool CleanupRobloxApp(const std::string& clientsDir, ProgressCallback progressCb) {
    if (progressCb) {
...
inline bool DownloadInsertDylib(const std::string& outputPath, ProgressCallback progressCb) {
...
inline bool DownloadDylib(const std::string& clientName, const std::string& outputPath,
                         ProgressCallback progressCb) {
    const std::string arch = GetHardwareArchitecture();
    ...
    if (clientName == "MacSploit") {
        ...
        LOG_INFO(std::format("Downloading dylib for MacSploit from {}", url));
        ...
        LOG_INFO(std::format("Fetching {} install script", clientName));

        const auto response = HttpClient::get(installUrl, {}, {}, true, 30);

        if (response.status_code != 200) {
            LOG_ERROR(std::format("Failed to fetch install script: HTTP {}", response.status_code));
            return false;
        }
        ...
        if (!std::regex_search(response.text, matches, urlRegex) || matches.size() < 2) {
            LOG_ERROR(std::format("Failed to parse {}.zip download URL from install script", clientName));
            return false;
        }

        const std::string zipUrl = matches[1].str();
        LOG_INFO(std::format("Found {}.zip URL: {}", clientName, zipUrl));
        ...
        LOG_INFO(std::format("Downloading {}.zip", clientName));
        ...
        if (!HttpClient::download(zipUrl, zipPath.string(), {}, zipProgress)) {
            LOG_ERROR(std::format("Failed to download {}.zip", clientName));
            return false;
        }
        ...
        if (!ExecuteCommand(unzipCmd, unzipOutput)) {
            LOG_ERROR(std::format("Failed to unzip {}: {}", clientName, unzipOutput));
        ...
        if (!std::filesystem::exists(appPath)) {
            LOG_ERROR(std::format("Failed to extract {}.app, application does not exist", clientName));
            std::filesystem::remove(zipPath);
            return false;
        }
        ...
        if (!std::filesystem::exists(sourceDylibPath)) {
            LOG_ERROR(std::format("Dylib not found at expected path: {}", sourceDylibPath.string()));
            std::filesystem::remove(zipPath);
        ...
        if (ec) {
            LOG_ERROR(std::format("Failed to move dylib: {}", ec.message()));
            std::filesystem::remove(zipPath);
            std::filesystem::remove_all(appPath);
            return false;
        }
        ...
        LOG_INFO(std::format("Successfully downloaded {} dylib", clientName));
        return true;
    }
    else if (clientName == "Delta") {
    	LOG_ERROR(std::format("Delta Client Not Yet Available"));
    	return false;
    }
    else {
        LOG_ERROR(std::format("Unknown client: {}", clientName));
        return false;
    }
}

inline bool InsertDylib(const std::string& insertDylibPath, const std::string& dylibPath,
    ...
    if (!ExecuteCommand(command, output)) {
        LOG_ERROR(std::format("Failed to insert dylib: {}", output));
        return false;
    }

    if (output.find("Added LC_LOAD_DYLIB") == std::string::npos) {
        LOG_ERROR(std::format("Unexpected insert_dylib output: {}", output));
        return false;
    }
...
inline bool CodeSign(const std::string& appPath, bool remove, ProgressCallback progressCb) {
    if (progressCb) {
    ...
    if (!ExecuteCommand(command, output)) {
        LOG_ERROR(std::format("Codesign failed: {}", output));
        return false;
    }
...
inline void InstallClientAsync(const std::string& clientName,
                               ProgressCallback progressCb,
    ...
    std::thread([clientName, progressCb, completionCb]() {
        ...
        if (arch == "aarch64" && clientName == "Delta") {
            ...
            try {
                ...
                LOG_ERROR(std::format("Failed to fetch Delta version: {}", e.what()));
            ...
            LOG_INFO(std::format("Installing Delta IPA version {}", version));
            ...
            if (success) {
                ...
                LOG_INFO(std::format("Successfully installed {} IPA", clientName));
        ...
        if (clientName == "Hydrogen") {
            try {
                ...
                LOG_WARN(std::format("Failed to fetch Hydrogen version: {}, falling back to latest", e.what()));
            }
            ...
            try {
                ...
                LOG_WARN(std::format("Failed to fetch MacSploit version: {}, falling back to latest", e.what()));
            }
        }
        ...
        LOG_INFO(std::format("Installing {} with Roblox version {}", clientName, version));
        ...
        std::filesystem::remove(zipPath, ec);
        if (ec) {
            LOG_WARN(std::format("Failed to cleanup zip file: {}", ec.message()));
        }
...
inline void RemoveClientAsync(const std::string& clientName, CompletionCallback completionCb) {
    std::thread([clientName, completionCb]() {
        ...
        if (ec) {
            LOG_ERROR(std::format("Failed to remove client: {}", ec.message()));
            if (completionCb) completionCb(false, std::format("Failed to remove: {}", ec.message()));
            return;
        }
        
        LOG_INFO(std::format("Removed client: {}", clientName));
