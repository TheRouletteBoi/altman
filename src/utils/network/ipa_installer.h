...
inline bool DownloadPackage(const std::string& appDataDir, const std::string& url,
    ...
    if (ec) {
        LOG_ERROR(std::format("Failed to create clients directory: {}", ec.message()));
        return false;
    }

    const std::filesystem::path clientPath = clientsDir / (client + ".ipa");

    LOG_INFO(std::format("Downloading {} from {}", client, url));
    ...
    return HttpClient::download(
        url,
        clientPath.string(),
        {{"User-Agent", std::format("RaptorManager/1.0")}},
        progress_adapter
    );
}

inline bool RestructureAppBundle(const std::filesystem::path& appDir, ProgressCallback progressCb) {
    if (progressCb) {
    ...
    if (ec) {
        LOG_ERROR(std::format("Failed to create MacOS directory: {}", ec.message()));
        return false;
    }

    std::filesystem::create_directories(frameworksDir, ec);
    if (ec) {
        LOG_ERROR(std::format("Failed to create Frameworks directory: {}", ec.message()));
        return false;
    }

    std::filesystem::create_directories(resourcesDir, ec);
    if (ec) {
        LOG_ERROR(std::format("Failed to create Resources directory: {}", ec.message()));
        return false;
    }
    ...
    if (std::filesystem::exists(robloxExec)) {
        ...
        if (ec) {
            LOG_ERROR(std::format("Failed to move Roblox executable: {}", ec.message()));
            return false;
        }
    } else {
    ...
    if (std::filesystem::exists(oldFrameworks)) {
        for (const auto& entry : std::filesystem::directory_iterator(oldFrameworks)) {
            ...
            if (ec) {
                LOG_ERROR(std::format("Failed to move {}: {}", entry.path().filename().string(), ec.message()));
                return false;
            }
        }
    ...
    if (std::filesystem::exists(oldPlist)) {
        ...
        if (ec) {
            LOG_ERROR(std::format("Failed to move Info.plist: {}", ec.message()));
            return false;
        }
    } else {
    ...
    for (const auto& entry : std::filesystem::directory_iterator(appDir)) {
        ...
        if (ec) {
            LOG_ERROR(std::format("Failed to move {} to Resources: {}", entry.path().filename().string(), ec.message()));
            return false;
        }
    }
...
inline bool InstallPackage(const std::string& appDataDir, const std::string& client,
                          ProgressCallback progressCb) {
    const std::filesystem::path clientsDir = std::filesystem::path(appDataDir) / "clients";
    const std::filesystem::path ipaPath = clientsDir / (client + ".ipa");

    if (!std::filesystem::exists(ipaPath)) {
        LOG_ERROR(std::format("Package {} not found. Anti-virus might be blocking.", client));
        return false;
    }
    ...
    if (!ExecuteCommand(unzipCmd, output)) {
        LOG_ERROR(std::format("Failed to unzip {}: {}", client, output));
        return false;
    }
    ...
    if (ec) {
        LOG_ERROR(std::format("Failed to rename app: {}", ec.message()));
        return false;
    }

    std::filesystem::remove_all(clientsDir / "Payload", ec);
    if (ec) {
        LOG_WARN(std::format("Failed to remove Payload dir: {}", ec.message()));
    }
    ...
    if (!ExecuteCommand(chmodCmd, output)) {
        LOG_ERROR(std::format("Failed to chmod: {}", output));
        return false;
    }

    std::filesystem::remove(ipaPath, ec);
    if (ec) {
        LOG_WARN(std::format("Failed to remove IPA: {}", ec.message()));
    }
...
inline bool Codesign(const std::filesystem::path& path, bool sign,
                    ProgressCallback progressCb) {
    if (progressCb) {
...
inline bool FixLibraryPaths(const std::filesystem::path& executablePath,
    ...
    if (std::filesystem::exists(libgloopPath)) {
        ...
        if (!ExecuteCommand(changeIdCmd, output)) {
            LOG_ERROR(std::format("Failed to change libgloop.dylib id: {}", output));
            return false;
        }
        ...
        if (!ExecuteCommand(changeRefCmd, output)) {
            LOG_ERROR(std::format("Failed to change libgloop reference in executable: {}", output));
            return false;
        }
...
inline bool FixRpath(const std::filesystem::path& executablePath, ProgressCallback progressCb) {
    if (progressCb) {
    ...
    if (!ExecuteCommand(addCmd, output)) {
        LOG_ERROR(std::format("Failed to add rpath: {}", output));
        return false;
    }
...
inline bool Convert(const std::filesystem::path& path, const std::string& name,
                   ProgressCallback progressCb) {
    if (progressCb) {
    ...
    std::string output;
    if (!ExecuteCommand(command, output)) {
        LOG_ERROR(std::format("Convert failed: {}", output));
        return false;
    }
...
inline bool ConvertPlist(const std::filesystem::path& path, ProgressCallback progressCb) {
    if (progressCb) {
    ...
    std::string output;
    if (!ExecuteCommand(command, output)) {
        LOG_ERROR(std::format("Convert plist failed: {}", output));
        return false;
    }
...
inline bool ModifyPlist(const std::filesystem::path& plistPath, ProgressCallback progressCb) {
    if (progressCb) {
...
inline bool InstallIPA(const std::string& appDataDir, const std::string& client,
    ...
    if (!std::filesystem::exists(robloxDir)) {
        LOG_ERROR(std::format("Package {} not found after install", client));
        return false;
    }
    ...
    LOG_INFO(std::format("Successfully installed {} IPA", client));
    return true;
}

inline void InstallIPAAsync(const std::string& appDataDir, const std::string& client,
                           const std::string& version, ProgressCallback progressCb,
