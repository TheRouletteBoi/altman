...
std::string formatRelativeFuture(time_t timestamp) {
    ...
    auto diff = std::chrono::duration_cast<std::chrono::seconds>(future - now).count();
    if (diff <= 0)
        return "now";
...
std::string formatCountdown(time_t timestamp) {
    ...
    auto now = std::chrono::system_clock::now();
    auto target = std::chrono::system_clock::from_time_t(timestamp);
    long long diff = std::chrono::duration_cast<std::chrono::seconds>(target - now).count();
...
std::string formatRelativeToNow(time_t timestamp) {
    auto now = std::chrono::system_clock::now();
    auto then = std::chrono::system_clock::from_time_t(timestamp);
    long long diffSeconds = std::chrono::duration_cast<std::chrono::seconds>(then - now).count();
